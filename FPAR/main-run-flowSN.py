...
def main_run(dataset, stage, train_data_dir, val_data_dir, stage1_dict, out_dir, seqLen, trainBatchSize,
    ...
    model_folder = os.path.join('./', out_dir, dataset, 'flow_surfaceNormals', 'stage'+str(stage))  # Dir for saving models and log files
    # Create the dir
        ...
        if val_data_dir is not None:
            if (epoch+1) % 1 == 0:
                ...
                if val_accuracy > min_accuracy:
                    save_path_model = (model_folder + '/model_flowSN_state_dict.pth')
                    torch.save(model.state_dict(), save_path_model)
                    min_accuracy = val_accuracy
            else:
                if (epoch+1) % 10 == 0:
                    save_path_model = (model_folder + '/model_flowSN_state_dict_epoch' + str(epoch+1) + '.pth')
                    torch.save(model.state_dict(), save_path_model)

...
def main_run(dataset, model_state_dict, dataset_dir, stackSize, seqLen, memSize):
    ...
    plt.grid(True)
    plt.clim(0, 1)
    plt.savefig(dataset + '-twoStreamJointDoubleResnet.jpg', bbox_inches='tight')
    plt.show()
...
def __main__():
    ...
    parser.add_argument('--modelStateDict', type=str, default='./models/gtea61/best_model_state_dict_twoStreamDoubleResnet_split2.pth',
                        help='Model path')

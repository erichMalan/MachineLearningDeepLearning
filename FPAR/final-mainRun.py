...
from finalObjectAttentionModelConvLSTM import *
from spatial_transforms import (Compose, ToTensor, CenterCrop, Scale, Normalize, MultiScaleCornerCrop,
                                RandomHorizontalFlip)
from tensorboardX import SummaryWriter
from finalMakeDataset import *
import argparse
import sys
import numpy as np
...
def main_run(dataset,stage, trainDatasetDir, valDatasetDir, stage1_dict, stackSize , out_dir, seqLen, trainBatchSize,
             valBatchSize, numEpochs, lr1, decay_factor, decay_step, memSize,alphaX,alphaY):
    ...
    vid_seq_train = makeDataset(trainDatasetDir,spatial_transform2,spatial_transform=spatial_transform,
    ...
    if valDatasetDir is not None:
        vid_seq_val = makeDataset(valDatasetDir,spatial_transform2,
    ...
    for epoch in range(numEpochs):
        ...
        train_log_loss.write('Training X loss after {} epoch= {}'.format(epoch+1,avg_x_loss))
        train_log_loss.write('Training Y loss after {} epoch= {}'.format(epoch+1,avg_y_loss))
        ...
        if valDatasetDir is not None:
            ...
            print('Val X Loss after {} epochs, loss = {}'.format(epoch + 1, avg_x_val_loss))
            print('Val Y Loss after {} epochs, loss = {}'.format(epoch + 1, avg_y_val_loss))
...
def __main__():
    ...
    parser.add_argument('--alphaX', type=float, default=1, help='Weight for the self supervised task of flowX')
    parser.add_argument('--alphaY', type=float, default=1, help='Weight for the self supervised task of flowY')
    ...
    main_run(dataset,stage, trainDatasetDir, valDatasetDir, stage1Dict, stackSize, outDir, seqLen, trainBatchSize,
             valBatchSize, numEpochs, lr1, decayRate, stepSize, memSize,alphaX,alphaY)

...
def main_run(dataset, trainDataset, valDataset, outDir, stackSize, trainBatchSize, valBatchSize, numEpochs, lr1,
    ...
    model_folder = os.path.join('./', outDir, dataset, 'flow_surfaceNormals_fm')  # Dir for saving models and log files
    # Create the dir
    ...
    model = flow_resnet34(True, channels=3*stackSize, num_classes=num_classes)
    model.train(True)
...
def __main__():
    ...
    main_run(dataset, trainDatasetDir, valDatasetDir, outDir, stackSize, trainBatchSize,

...
def main_run(dataset, flowModel_state_dict, RGBModel_state_dict, dataset_dir, stackSize, seqLen, memSize, numSeg):
    ...
    modelRGBSN = attentionMDoubleResnet(num_classes=num_classes, mem_size=memSize)
    ...
    for j, (inputFlow, inputFrame, inputSN, targets) in enumerate(test_loader):
        ...
        inputSN = Variable(inputSN.permute(1, 0, 2, 3, 4).cuda(), volatile=True)
        output_labelFlow, _ = modelFlow(inputVariableFlow)
        output_labelFrameSN, _ = modelRGBSN(inputVariableFrame,inputVariableSN)
        output_label_meanFlow = torch.mean(output_labelFlow.data, 0, True)
        output_label_meanTwoStream = (flow_wt * output_label_meanFlow) + ((1-flow_wt) * output_labelFrameSN.data)
...
def __main__():
    ...
    parser.add_argument('--RGBSNModelStateDict', type=str, default='./models/gtea61/best_model_state_dict_rgbSN_split2.pth',
                        help='RGB Model path')

...
class makeDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for i in np.linspace(1, numFrame, self.seqLen, endpoint=False):
            ...
            fl2_name = vid_nameF +  '/' + 'flow_surfaceNormals' +'/' + 'flow_surfaceNormal_' + str(int(np.floor(i))).zfill(4) + self.fmt
            img2 = Image.open(fl2_name)
            inpSeqSN.append(self.spatial_transform(img2.convert('RGB')))
        inpSeqF = torch.stack(inpSeqF, 0)
        inpSeqSN = torch.stack(inpSeqSN, 0)
        return inpSeqSegs, inpSeqF, inpSeqSN, label#, vid_nameF#, fl_name

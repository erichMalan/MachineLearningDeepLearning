...
def main_run(stage, train_data_dir, val_data_dir, stage1Dict, stage1Dict_rgb, stage1Dict_fc, out_dir, seqLen, trainBatchSize,
    ...
    model_folder = os.path.join('./', out_dir, 'crossAttConvLSTMDoubleResnet', str(seqLen), 'stage'+str(stage)) # Dir for saving models and log files
    # Create the dir
    ...
    train_params = []
    if stage == 1:
        model = crossAttentionDoubleResnet(num_classes=num_classes, mem_size=memSize, rgbm = stage1Dict_rgb,  fcm = stage1Dict_fc)
        model.train(False)
...
def __main__():
    ...
    parser.add_argument('--rgbModel', type=str, default=None,
                        help='model path')
    parser.add_argument('--stage1Dict', type=str, default=None,
                        help='Stage 1 model path')
    parser.add_argument('--fcModel', type=str, default=None,
                        help='model path')

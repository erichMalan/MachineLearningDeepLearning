...
from makeDatasetTwoStreamDoubleResnet import *
import argparse
...
def main_run(dataset, flowModel, rgbSNModel, stackSize, seqLen, memSize, trainDatasetDir, valDatasetDir, outDir,
    ...
    model_folder = os.path.join('./', outDir, dataset, 'twoStreamDoubleResnet')  # Dir for saving models and log files
    # Create the dir
    ...
    model = twoStreamAttentionModel(flowModel=flowModel, frameModel=rgbSNModel, stackSize=stackSize, memSize=memSize,
    ...
    for epoch in range(numEpochs):
        ...
        for j, (inputFlow, inputFrame, inputSN, targets) in enumerate(train_loader):
            train_iter += 1
            iterPerEpoch += 1
            optimizer_fn.zero_grad()
            inputVariableFlow = Variable(inputFlow.cuda())
            inputVariableFrame = Variable(inputFrame.permute(1, 0, 2, 3, 4).cuda())
            inputVariableSN = Variable(inputSN.permute(1, 0, 2, 3, 4).cuda())
            labelVariable = Variable(targets.cuda())
            output_label = model(inputVariableFlow, inputVariableFrame, inputVariableSN)
        ...
        if valDatasetDir is not None:
            if (epoch + 1) % 1 == 0:
                ...
                val_iter = 0
                numCorr = 0
                for j, (inputFlow, inputFrame, inputSN, targets) in enumerate(val_loader):
                    val_iter += 1
                    inputVariableFlow = Variable(inputFlow.cuda())
                    inputVariableFrame = Variable(inputFrame.permute(1, 0, 2, 3, 4).cuda())
                    inputVariableSN = Variable(inputSN.permute(1, 0, 2, 3, 4).cuda())
                ...
                if val_accuracy > min_accuracy:
                    save_path_model = (model_folder + '/model_twoStreamDoubleResnet_state_dict.pth')
                    torch.save(model.state_dict(), save_path_model)
                    min_accuracy = val_accuracy
        else:
            if (epoch + 1) % 10 == 0:
                save_path_model = (model_folder + '/model_twoStreamDoubleResnet_state_dict_epoch' + str(epoch + 1) + '.pth')
                torch.save(model.state_dict(), save_path_model)
...
def __main__():
    ...
    parser.add_argument('--flowModel', type=str, default='./experiments/gtea61/flow/best_model_state_dict.pth',
                        help='Flow model')
    parser.add_argument('--rgbSNModel', type=str, default='./experiments/gtea61/rgbSN/best_model_state_dict.pth',
                        help='RGB model')
    ...
    main_run(dataset, flowModel, rgbSNModel, stackSize, seqLen, memSize, trainDatasetDir, valDatasetDir, outDir,
             trainBatchSize, valBatchSize, lr1, numEpochs, decay_step, decay_factor)
